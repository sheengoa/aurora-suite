...
Page({
  ...
  async confirmBind() {
    ...
    try {
      ...
      // 调用云函数绑定打印机
      const bindRes = await callAdminFunction('printManage', {
        $url: 'addPrinter',
        sn: sn.trim(),
        key: key.trim(),
  ...
  async doUnbindPrinter() {
    try {
      ...
      // 调用云函数解绑
      const unbindRes = await callAdminFunction('printManage', {
        $url: 'delPrinter',
        sn: [printerInfo.sn]
      })
  ...
  async queryStatus() {
    ...
    try {
      const res = await callAdminFunction('printManage', {
        $url: 'getDeviceStatus',
        sn: printerInfo.sn
      })
  ...
  async testPrint() {
    ...
    try {
      ...
      // 显示支付方式
      let payMethodText = ''
      if (testOrder.payWithBalance !== undefined) {
      ...
      // 调用打印接口
      const printRes = await callAdminFunction('printManage', {
        $url: 'printNote',
        sn: printerInfo.sn,
        voice: '16',
        voicePlayTimes: 1,
